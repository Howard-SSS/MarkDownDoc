...
二进制形式记录数据库的各种更改操作，但不记录查询语句

```mysql
mysqlbinlog FILENAME # 查看二进制日志
```

**错误日志**

记录启动、关闭和运行错误等信息，**默认启动**

```mysql
mysqladmin -uroot -p flush-logs # 开启新的错误日志
```

**通用查询日志**

记录启动、关闭、客户端连接、更新、查询的SQL语句

**慢查询日志**

记录执行事件操作指定时间的操作，默认超过10秒记录时记录
